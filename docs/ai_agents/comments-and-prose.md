...
PowerShell and workflow comments follow the same burden. Keep a gate ordering,
...
the schema contract is intentionally changing, and run the validation gate.
...
```powershell
# Verify vendor hashes before any gate consumes the promoted fixtures.
```
