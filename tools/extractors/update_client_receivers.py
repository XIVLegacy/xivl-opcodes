...
def update_apply_chain(firers_path: Path) -> int:
    ...
    recvs["apply_chain_note"] = (
        "BCS-Y-0351 defines the apply_chain binding mechanism: a Lua name fired from a "
        "receiver's apply helper at depth two or greater rather than from a "
        "LuaActorImpl vtable-slot dispatcher. Each apply_chain_lua_readers "
        "entry stores the {luaName, opcode, applyHelperVa, fireSite, "
        "evidenceBcsy} relationship."
    )
...
def update_indirect(catalog_path: Path) -> int:
    ...
    recvs["indirect_lua_readers_note"] = (
        "BCS-Y-0338, BCS-Y-0341, BCS-Y-0342, and BCS-Y-0343 define the "
        "indirect-binding evidence for each receiver that writes an "
        "actor-state field also read by a Lua API. An indirect_lua_readers "
        "entry stores the {luaName, sharedField, luaApiImplVa, bcsRefs} "
        "relationship. This mechanism is distinct from direct _onXxx callback "
        "firing through lua_actor_impl_slot."
    )
