...
entry point, payload-framing contract, and limits on what validation proves. Run
the checks that cover the change and report any unverified edge plainly. CI
runs the repository checks on every pull request, and all checks must pass
before merge.
